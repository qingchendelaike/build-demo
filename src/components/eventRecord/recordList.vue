<template>
  <el-container class="container-box record-wrapper">
    <el-main class="main-box" id="scrollContainer">
      <el-timeline>
        <el-timeline-item v-for="(item,index) in modelData" :key="index" :class="{'today':isToday(item.date)}" :timestamp="item.date" placement="top">
       ...
       <el-collapse-transition>
        <span v-show="showDate">
          ...
          <el-date-picker
            v-model="value1"
            type="date"
            placeholder="请选择开始日期" value-format="yyyy-MM-dd" @change="refreshData()">
          </el-date-picker>
        </span>
         </el-collapse-transition>
        <el-button @click="toggleDate()"><span class="location-icon"></span></el-button>
    </span>
